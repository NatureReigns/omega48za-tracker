...
function App() {
  ...
  useEffect(() => {
    if (user) {
      const calculateWeeklyPayout = async () => {
        ...
        const weekTotal = (weekSales || []).reduce((sum, s) => sum + s.amount_zar, 0);
        const payout = weekTotal * ((commissionRate + bonusRate) / 100) + overrideEarnings;
  ...
  if (user) {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `agent_id=eq.${user.id}` }, loadNotifications)
      .subscribe();
